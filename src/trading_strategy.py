...
class TradingStrategy(object):
    ...
    def compute_lead_lag_transform(self, Z: np.ndarray) -> np.ndarray:
        """
        This method computes the lead-lag transform Z^LL_t of each market factor process Z_t in the batch Z.
        """
        pass
    ...
    def compute_functionals(self, mu_sig: np.ndarray, sigma_sig: np.ndarray) -> np.ndarray:
        """
    ...
    def fit(self, X: np.ndarray, f: np.ndarray) -> None:
        ...
        """
        assert X.shape[0] == f.shape[0], "Number of price paths and factor paths must be the same"
        ...
        N = f.shape[2] # number of non-tradable factors (i.e. dimension of the factor paths)
        ...
        Z_LL = self.compute_lead_lag_transform(Z)

        # compute the N-truncated signature ZZ^^LL_t of each lead-lag transform Z^LL_t
        ZZ_LL = utils.compute_signature(Z_LL, self.N)

        # compute the expected N-truncated signature E[ZZ^^LL_t] using the empirical mean
        E_ZZ_LL = np.mean(ZZ_LL, axis=0)

        # compute the mu_sig vector as defined in the paper
        mu_sig = self.compute_mu_sig(E_ZZ_LL)

        # compute the simga_sig matrix as defined in the paper
        sigma_sig = self.compute_sigma_sig(E_ZZ_LL)

        # now we can finally compute the functionals l_m for m = 1, ..., d
    ...
    def trade(self, X: np.ndarray, f: np.ndarray) -> np.ndarray:
        ...
        """
        assert self.functionals is not None, "The model must be fitted before calling the trade method"
        ...
        N = f.shape[1] # number of non-tradable factors (i.e. dimension of the factor paths)
    ...
    def compute_pnl(self, X: np.ndarray, xi: np.ndarray) -> float:
        ...
        """
        assert X.shape[0] == xi.shape[0]+1, "xi must have exactly one less time step than X"
